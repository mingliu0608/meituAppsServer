...
const { Pics, detailPage } = require('../db/models')
const fs = require('fs')
...
//get data
const getPicData = (page = 1, type, cat) => {
  return new Promise((resolve, reject) => {
    try {
      //妹子网的爬取逻辑
      c.queue([
        {
          uri: 'https://www.mzitu.com/' + cat + '/page/' + page,
            ...
            if (error) {
              ...
              rawData.each((index, element) => {
                let datas = {
                  ...
                  desrc: element.attribs.alt,
                  dataOriginal: element.attribs['data-original'],
                  id: generatorId(),
                  //获父节点的a标签并且href属性
                  pageUrl: element.parent.attribs.href,
                  type: type,
                }
                let newPics = new Pics(datas)
                newPics.save((err) => {
                  if (err) {
                    console.log(err)
                  }
...
//获取首页信息，得到每页的url
async function getAllPage(startPage = 1, endPage, type = '首页', cat = '') {
  // let datas = {}
  try {
    for (i = startPage; i < endPage; i++) {
      await getPicData(i, type, cat).then((res) => {
        // console.log(res)
